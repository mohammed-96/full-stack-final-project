...
APPLICATION_NAME = "Company Menu Application"
...
def createUser(login_session):
    ...
    Usera = User(email=str(login_session['email']))
    session.add(Usera)
    session.commit()
    user = session.query(User).filter_by(
        email=str(login_session['email'])).one()
    return user.id
...
def showCategories():
    ...
    items = session.query(CategoryItem).order_by(
            CategoryItem.id.desc()).limit(9).all()
...
def showItems(Category_name):
    ...
    categoryid = session.query(Category).filter_by(name=Category_name).one().id
    ...
    if request.method == 'POST':
        if (
            ...
            categoryid = session.query(Category).filter_by(
                name=request.form.get("CategoryName")).one().id
